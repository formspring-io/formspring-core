...
export class Client {
  ...
  submitForm(
    ...
    let endpoint = opts.endpoint || 'https://formspring.io';
    ...
    let headers: { [key: string]: string } = {
      ...
      'Formspring-Client': clientHeader(opts.clientName)
    };

    if (this.session) {
      headers['Formspring-Session-Data'] = encode64(this.session.data());
    }
